...
```sh
yarn test          # watch 模式
yarn test:run      # 默认并行四个 shard 单次运行
...
`yarn test:run` 默认并行执行四个 Vitest shard，本地全量测试和 CI 使用同一个入口。传入 `--shard=1/4` 之类的显式分片参数时只执行对应 shard；`yarn test:run --serial` 强制单进程运行。传入测试文件或名称过滤条件时自动使用单进程，避免聚焦测试因文件数不足而无法分片。

`Frontend Tests` 工作流使用 Node 24 LTS 和 frozen lockfile，在面向 `v3_plus` 的 Pull Request 和手动触发时运行（fork 侧关闭 push 直推以控制 Actions 额度，见 `test.yml` 触发块注释）。`lint` 与 `typecheck` job 分别执行全仓只读 ESLint 和类型检查，并与四个 Vitest shard 并行运行。单元测试 matrix 通过统一入口执行，成功用例的标准输出默认静默，失败详情仍保留。CI 不要求覆盖率达标；覆盖率可按需在本地运行 `yarn test:coverage`。变更文件格式检查依赖 Pull Request 的 base/head SHA，因此只在 Pull Request 事件运行。

Pull Request 始终执行完整门禁；所有检查成功后，`CI proof` job 将实际测试的模拟合并提交 SHA 记录在 job 名称中。合并后的 push 先执行轻量 `reuse` 检查：只有对应 PR 的同一工作流最新运行成功、模拟合并提交基于本次 push 前的目标分支、PR head 一致且代码树完全相同，才跳过重复的 lint、typecheck 和单元测试。该判定同时适用于 merge、squash 和 rebase 后代码相同的合并；直接 push、手动触发、强制推送、目标分支变化、证明缺失或 GitHub API 查询失败均继续执行全量检查。首次上线前的旧 PR 没有 `CI proof`，合并后也会补跑全量。fork 侧不再由 push 触发本工作流，`reuse` 逻辑保留但仅在手动触发时可达。`build.yml` 的构建发布触发规则保持独立。

Prettier 和 Node 兼容范围按[前端代码质量工具链演进](code-quality.md)继续渐进接入，新增测试代码不得引入新的 lint 或格式问题。
