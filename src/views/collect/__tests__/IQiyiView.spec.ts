...
async function renderSourceView() {
  const result = renderWithProviders(IQiyiView, {
    global: { stubs: { MediaCardListView: CardListStub, MediaSearchView: true } },
  })
  await waitFor(() => {
    expect(mocks.apiGet).toHaveBeenCalledWith('iqiyi/category', expect.anything())
  })
  return result
}

describe('IQiyiView（六源同构契约）', () => {
  beforeEach(() => {
    mocks.apiGet.mockReset()
    mocks.apiGet.mockImplementation((endpoint: string) => {
      if (endpoint === 'iqiyi/category') return categoryFixture()
      if (endpoint === 'iqiyi/user/info') return { is_login: true, vip_status: 1 }
      return []
    })
  })

  it('挂载即拉取 iqiyi/category 分类', async () => {
  ...
  it('媒体列表接线到本源的 page_data 端点', async () => {
    ...
    expect((container.querySelector('.media-card-list-stub') as HTMLElement).dataset.apipath).toBe('iqiyi/page_data')
  })
      it('拉取并展示登录用户信息', async () => {
        await renderSourceView()
        expect(mocks.apiGet).toHaveBeenCalledWith('iqiyi/user/info')
      })
