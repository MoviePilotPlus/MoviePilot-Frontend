...
async function checkLoginStatus(type: 'tv' | 'web') {
  try {
    ...
    const statusRef = type === 'tv' ? tvLoginStatus : webLoginStatus
    const userInfoRef = type === 'tv' ? tvUserInfo : webUserInfo
...
async function getQRCode() {
  try {
    ...
    qrCodeKey.value = response.qrcode_key
...
function startPolling() {
  ...
  pollingTimer.value = window.setInterval(async () => {
    try {
      ...
      const response: { [key: string]: any } = await pluginApi.get(`bilibili/login_status/${loginType.value}`, {
