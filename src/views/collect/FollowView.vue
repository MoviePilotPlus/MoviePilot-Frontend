...
async function loadFollowTasks() {
  loading.value = true
  try {
    followTasks.value = await api.get('follow/')
  } catch (error) {
...
async function manualCheck(taskId: number) {
  try {
    $toast.info('正在检测...')
    await api.post(`follow/${taskId}/check`)
    loadFollowTasks()
  } catch (error) {
...
async function toggleTask(task: FollowTask) {
  try {
    await api.post(`follow/${task.id}/toggle`)
    loadFollowTasks()
  } catch (error) {
...
async function deleteTask(taskId: number) {
  ...
  try {
    await api.delete(`follow/${taskId}`)
    $toast.success('删除成功')
    loadFollowTasks()
  } catch (error) {
...
async function showRecords(task: FollowTask) {
  try {
    records.value = await api.get(`follow/${task.id}/records`)
