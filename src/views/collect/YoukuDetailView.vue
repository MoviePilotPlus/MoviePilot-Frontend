...
async function loadDetail() {
  ...
  errorMessage.value = ''
  try {
    const data = (await api.get('youku/detail', {
...
function openSeriesPage() {
  if (!props.cid) return
  window.open(`https://v.youku.com/video?s=${props.cid}`, '_blank')
}
...
function openEpisode(episode: TencentEpisodeInfo | null) {
  ...
  if (episode.vid) {
    window.open(`https://v.youku.com/v_show/id_${episode.vid}.html`, '_blank')
  }
}
        ...
        <VCol cols="12" md="9">
          ...
          <div class="d-flex flex-wrap ga-2">
            ...
            <VBtn variant="outlined" prepend-icon="mdi-open-in-new" @click="openSeriesPage">
              {{ t('collect.detail.openYoukuPage') }}
            </VBtn>
