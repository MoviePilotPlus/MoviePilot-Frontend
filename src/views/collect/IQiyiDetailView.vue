...
const seriesUrl = computed(() => {
  const episodeUrl = episodes.value.find(item => item.web_play_url)?.web_play_url
...
async function loadDetail() {
  ...
  errorMessage.value = ''
  try {
    const data = (await api.get('iqiyi/detail', {
...
function openSeriesPage() {
  if (!seriesUrl.value) return
  window.open(seriesUrl.value, '_blank')
}
...
<template>
  ...
  <div v-else-if="detail" class="px-3">
        ...
        <VCol cols="12" md="9">
          ...
          <div class="d-flex flex-wrap ga-2">
            ...
            <VBtn variant="outlined" prepend-icon="mdi-open-in-new" @click="openSeriesPage">
              {{ t('collect.detail.openIqiyiPage') }}
            </VBtn>
    ...
    <VCard class="mb-4">
      <VCardTitle>{{ t('collect.detail.definition') }}</VCardTitle>
      <VCardText>
        <VChipGroup column>
          <VChip v-for="(item, idx) in detail.definition_list || []" :key="item.id || item.name || idx" size="small">
            {{ item.cname || item.name || item.id }}
          </VChip>
        </VChipGroup>
      </VCardText>
    </VCard>
