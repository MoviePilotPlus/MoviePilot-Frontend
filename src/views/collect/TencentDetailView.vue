...
async function loadDetail() {
  ...
  errorMessage.value = ''
  try {
    const data = (await api.get('tencent/detail', {
...
function openSeriesPage() {
  if (!props.cid) return
  window.open(`https://v.qq.com/x/cover/${props.cid}.html`, '_blank')
}
...
function openEpisode(episode: TencentEpisodeInfo | null) {
  ...
  if (episode.cid && episode.vid) {
    window.open(`https://v.qq.com/x/cover/${episode.cid}/${episode.vid}.html`, '_blank')
  }
}
        ...
        <VCol cols="12" md="9">
          ...
          <div class="d-flex flex-wrap ga-2">
            ...
            <VBtn variant="outlined" prepend-icon="mdi-open-in-new" @click="openSeriesPage">
              {{ t('collect.detail.openTencentPage') }}
            </VBtn>
