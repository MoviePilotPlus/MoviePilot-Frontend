...
const zhTW = {
  ...
  setting: {
    collect: {
      ...
      screenshotCompressLimit: '截圖單張大小上限（位元組）',
      ...
      doubanCookiesHint: '豆瓣 IP 間歇風控時配置登入態 Cookies 緩解（關鍵是 dbcl2），如 dbcl2="xxx"; bid="yyy"；留空自動生成隨機 bid 訪客態',
      doubanCookiesPlaceholder: 'dbcl2="xxxxx"; bid="yyyyy"',
      doubanUseProxy: '豆瓣請求走代理',
      doubanUseProxyHint: '豆瓣請求是否經 PROXY_HOST 代理出口，IP 被風控時可開啟（需先在設定-服務中配置代理）',
      doubanBrowserFallback: '豆瓣攔截回退瀏覽器',
      doubanBrowserFallbackHint: 'requests 請求被豆瓣攔截（返回異常頁）時是否回退真實瀏覽器重新請求',
      ptgenSource: '簡介抓取線路',
      ptgenSourceDesc: 'PTGen 簡介抓取的資料源線路與優先級（豆瓣 IP 風控時鏡像線路保證不斷流）',
      ptgenSourceOrderHint: '拖動調整優先級：豆瓣直連失敗後按此順序嘗試鏡像線路；豆瓣直連資料最全始終最先，關閉鏡像線路則直接跳過',
      ptgenSourceDouban: '豆瓣直連',
      ptgenSourceRefactor: 'PT-Gen-Refactor',
      ptgenSourceRefactorDesc: 'PT-Gen-Refactor 遠端服務（直連源站解析，完整度接近直連；支援豆瓣/IMDb/TMDB 等）',
      ptgenSourceRefactorBaseUrl: 'PT-Gen-Refactor 服務地址（可選）',
      ptgenSourceRefactorBaseUrlHint: '自建部署地址；留空用官方實例 https://pt-gen.hares.dpdns.org',
      ptgenSourceRefactorSecret: 'PT-Gen-Refactor 簽名密鑰（可選）',
      ptgenSourceRefactorSecretHint: '自建部署的 AUTH_SECRET；留空用官方實例內置密鑰',
      ptgenSourceDoubanDesc: '豆瓣頁面直連 + 瀏覽器渲染，資料最全（評分/想看數/演職員完整），始終最先嘗試',
      ptgenSourceWmdb: 'WMDB 鏡像',
      ptgenSourceWmdbDesc: 'api.wmdb.tv 豆瓣資料鏡像，核心欄位可用（演員表常缺）',
      ptgenSourceSaveSuccess: '簡介抓取線路已保存',
      torrentAuthor: '種子作者',
      torrentAuthorHint: '製作種子時寫入作者資訊（comment 與匿名發布時的作者兜底），留空不添加',
      ...
      tencentFetchLine: '騰訊採集線路',
      tencentFetchLineHint: '選擇騰訊視頻採集時使用的線路（部分版權內容僅手機App形態放行）',
      tencentFetchLineSaveSuccess: '騰訊採集線路保存成功',
      ...
      pixhost: 'Pixhost（免帳號）',
      priorityLabel: '優先級',
      hostingIpicDesc: '需賬號，接口穩定',
      hostingImgbbDesc: '需 API Key，國內可達',
      hostingPandaDesc: 'AilMWeb 製作組固定使用',
      hostingImgboxDesc: '需賬號密碼',
      hostingPixhostDesc: '免賬號，單圖上限 10MB',
      siteSchema: '站點模闆',
      ...
      siteSchemaSyncConfirmTitle: '同步覆蓋站點模板',
      siteSchemaSyncConfirmText: '將用默認 siteschema.json 覆蓋數據庫中的站點模板：已有同域名的模板會被覆蓋（2FA 密鑰保留），數據庫中獨有的模板不受影響。確定繼續？',
      siteSchemaSyncSuccess: '覆蓋同步完成：寫入 {written} 條，庫中獨有保留 {kept} 條',
      siteSchemaSyncFailed: '站點模板覆蓋同步失敗',
