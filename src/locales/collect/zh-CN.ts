...
const zhCN = {
  ...
  setting: {
    collect: {
      ...
      screenshotCompressLimit: '截图单张大小上限（字节）',
      ...
      doubanCookiesHint: '豆瓣 IP 间歇风控时配置登录态 Cookies 缓解（关键是 dbcl2），如 dbcl2="xxx"; bid="yyy"；留空自动生成随机 bid 访客态',
      doubanCookiesPlaceholder: 'dbcl2="xxxxx"; bid="yyyyy"',
      doubanUseProxy: '豆瓣请求走代理',
      doubanUseProxyHint: '豆瓣请求是否经 PROXY_HOST 代理出口，IP 被风控时可开启（需先在设定-服务中配置代理）',
      doubanBrowserFallback: '豆瓣拦截回退浏览器',
      doubanBrowserFallbackHint: 'requests 请求被豆瓣拦截（返回异常页）时是否回退真实浏览器重新请求',
      ptgenSource: '简介抓取线路',
      ptgenSourceDesc: 'PTGen 简介抓取的数据源线路与优先级（豆瓣 IP 风控时镜像线路保证不断流）',
      ptgenSourceOrderHint: '拖动调整优先级：豆瓣直连失败后按此顺序尝试镜像线路；豆瓣直迂数据最全始终最先，关闭镜像线路则直接跳过',
      ptgenSourceDouban: '豆瓣直连',
      ptgenSourceRefactor: 'PT-Gen-Refactor',
      ptgenSourceRefactorDesc: 'PT-Gen-Refactor 远端服务（直连源站解析，完整度接近直连；支持豆瓣/IMDb/TMDB 等）',
      ptgenSourceRefactorBaseUrl: 'PT-Gen-Refactor 服务地址（可选）',
      ptgenSourceRefactorBaseUrlHint: '自建部署地址；留空用官方实例 https://pt-gen.hares.dpdns.org',
      ptgenSourceRefactorSecret: 'PT-Gen-Refactor 签名密钥（可选）',
      ptgenSourceRefactorSecretHint: '自建部署的 AUTH_SECRET；留空用官方实例内置密钥',
      ptgenSourceDoubanDesc: '豆瓣页面直连 + 浏览器渲染，数据最全（评分/想看数/演职员完整），始终最先尝试',
      ptgenSourceWmdb: 'WMDB 镜像',
      ptgenSourceWmdbDesc: 'api.wmdb.tv 豆瓣数据镜像，核心字段可用（演员表常缺）',
      ptgenSourceSaveSuccess: '简介抓取线路已保存',
      torrentAuthor: '种子作者',
      torrentAuthorHint: '制作种子时写入作者信息（comment 与匿名发布时的作者兜底），留空不添加',
      ...
      tencentFetchLine: '腾讯采集线路',
      tencentFetchLineHint: '选择腾讯视频采集时使用的线路（部分版权内容仅手机App形态放行）',
      tencentFetchLineSaveSuccess: '腾讯采集线路保存成功',
      ...
      pixhost: 'Pixhost（免账号）',
      priorityLabel: '优先级',
      hostingIpicDesc: '需账号，接口稳定',
      hostingImgbbDesc: '需 API Key，国内可达',
      hostingPandaDesc: 'AilMWeb 制作组固定使用',
      hostingImgboxDesc: '需账号密码',
      hostingPixhostDesc: '免账号，单图上限 10MB',
      siteSchema: '站点模板',
      ...
      siteSchemaSyncConfirmTitle: '同步覆盖站点模板',
      siteSchemaSyncConfirmText: '将用默认 siteschema.json 覆盖数据库中的站点模板：已有同域名的模板会被覆盖（2FA 密钥保留），数据库中独有的模板不受影响。确定继续？',
      siteSchemaSyncSuccess: '覆盖同步完成：写入 {written} 条，库中独有保留 {kept} 条',
      siteSchemaSyncFailed: '站点模板覆盖同步失败',
