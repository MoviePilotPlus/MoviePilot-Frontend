...
import type { SiteSchema } from '@/api/types'
...
// 导入的文件数据
const importData = ref<SiteSchema[]>([])

// 导入进度
const importProgress = ref(0)

// 预览数据
const previewData = ref<SiteSchema[]>([])

// 选中的文件
const selectedFile = ref<File | null>(null)

// 导入错误信息
const importErrors = ref<Array<{ site: SiteSchema; error: string }>>([])

// 导入成功的站点
const importSuccesses = ref<SiteSchema[]>([])
  ...
  try {
    ...
    for (let i = 0; i < validSites.length; i++) {
      ...
      try {
        ...
        const result: { success: boolean; message?: string } = await api.post('siteschema/', siteData)
        if (result.success) {
          // 记录成功的站点
          successCount++
          ...
          importErrors.value.push({
            site,
            error: result.message || t('setting.collect.siteSchemaImportFailed'),
          })
        }
      } catch (error) {
        console.error(`Import site schema ${site.name} failed:`, error)
        failCount++
        // 记录错误信息
        importErrors.value.push({
          site,
          error: error instanceof Error ? error.message : t('setting.collect.siteSchemaImportFailed'),
        })
      }
      // 更新进度
    ...
    if (failCount === 0 && successCount > 0) {
      // 全部成功，直接关闭对话框
      $toast.success(t('setting.collect.siteSchemaImportSuccess', { count: successCount }))
      closeDialog(true)
    ...
    $toast.error(t('setting.collect.siteSchemaImportFailed'))
    // 出错时回到预览阶段
    currentStage.value = ImportStage.PREVIEW_FILE
  } finally {
    doneNProgress()
  ...
  <VDialog scrollable max-width="50rem" :fullscreen="!display.mdAndUp.value">
    <VCard>
      <VCardItem class="py-2">
        ...
        <VCardSubtitle>{{ t('setting.collect.siteSchemaImportHint') }}</VCardSubtitle>
      </VCardItem>
        ...
        <div v-if="currentStage === ImportStage.IMPORT_COMPLETE" class="result-area">
          ...
          <div v-if="importSuccesses.length > 0" class="success-sites mb-4">
            <VAlert
              ...
              variant="tonal"
              class="mb-4"
              :text="t('setting.collect.siteSchemaImportSuccess', { count: importSuccesses.length })"
            />
          </div>
