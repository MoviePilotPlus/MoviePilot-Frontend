...
<template>
  <VDialog scrollable fullscreen :scrim="false" transition="dialog-bottom-transition">
    <VCard>
      ...
      <div class="p-3">
        <VRow>
          ...
          <VCol cols="6" md="5">
            <VSelect v-model="selectCategory" :items="categoryOptions" size="small" density="compact" chips label="资源分类"
              multiple clearable />
          </VCol>
      ...
      <VCardText class="px-0 py-0 my-0">
        <VDataTable v-model:items-per-page="resourceItemsPerPage" :headers="resourceHeaders" :items="resourceDataList"
          :items-length="resourceTotalItems" :search="resourceSearch" :loading="resourceLoading" density="compact"
          item-value="title" return-object fixed-header hover items-per-page-text="每页条数" page-text="{0}-{1} 共 {2} 条"
          loading-text="加载中..." class="h-full">
          <template #item.title="{ item }">
            <a href="javascript:void(0)" @click.stop="addDownload(item)">
              ...
              <VChip v-for="(label, index) in item.labels" :key="index" variant="elevated" size="small" color="primary"
                class="me-1 mb-1">
                {{ label }}
              </VChip>
              <VChip v-if="item.downloadvolumefactor !== 1 || item.uploadvolumefactor !== 1"
                :class="getVolumeFactorClass(item.downloadvolumefactor, item.uploadvolumefactor)" variant="elevated"
              ...
              <IconBtn>
                ...
                <VMenu activator="parent" close-on-content-click>
                  <VList>
                    ...
                    <VListItem v-if="item.enclosure?.startsWith('http')" variant="plain"
    ...
    <!-- 添加下载对话框 -->
    <AddDownloadDialog v-if="addDownloadDialog" v-model="addDownloadDialog" :torrent="torrent"
      @done="addDownloadSuccess" @error="addDownloadError" @close="addDownloadDialog = false" />
  </VDialog>
</template>
