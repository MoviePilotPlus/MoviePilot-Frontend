...
async function removeIgnore() {
  ...
  startNProgress()
  try {
    await api.delete(`collect/ignore/${props.media?.source}/${props.media?.cid}`)
...
async function addIgnore() {
  ...
  startNProgress()
  try {
    await api.post(`collect/ignore/${props.media?.source}/${props.media?.cid}`)
